...
def extract_local_IP(timeout = 0):
    """
    This method retrieves a list of network interfaces and polls them to find the current in use local
    address. The IP is assumed to be a standard IPv4 "class" A, B, or C address
    """
    ...
    for current in interfaces:
        for i in current:
            if i.isalpha() == False:
                if i.startswith("127.") == False and i.startswith("169.") == False: # internal and loopback
